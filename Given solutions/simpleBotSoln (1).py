...
class Bot():
    ...
    def __init__(self,namep):
        ...
        self.vl = 5.0
        self.vr = 5.0
        self.timeSinceLastTurn = 0
        self.timeToTurn = -999 #meaningless initial value
    ...
    def transferFunction(self, lightL, lightR):
        ...
        # speed depends on nothing - straight line
        #self.vl = 5.0
        #self.vr = 5.0

        # speed depends on nothing - turning in a circle
        #self.vl = 3.0
        #self.vr = 5.0

        # speed depends on nothing - turning on the spot
        #self.vl = -5.0
        #self.vr = 5.0

        # random speed changes - moves around randomly
        #self.vl = random.uniform(-5.0,5.0)
        #self.vr = random.uniform(-5.0,5.0)
        
        # speed depends on light intensity only (vehicle 2c - similar to vehicle 1)
        #self.vl = (lightL+lightR)
        #self.vr = (lightL+lightR)
        
        # attracted to light (vehicle 2b)
        self.vl = lightR
        self.vr = lightL

        # repelled by light (vehicle 2a)
        #self.vl = lightL
        #self.vr = lightR

        # wander and turn
        # if self.timeSinceLastTurn<30:
        #     self.timeSinceLastTurn += 1
        # elif self.timeSinceLastTurn==30:
        #     self.timeSinceLastTurn += 1
        #     self.timeToTurn = random.randrange(5,30)
        #     self.vl = 5.0
        #     self.vr = -5.0
        # elif self.timeToTurn>0:
        #     self.timeSinceLastTurn += 1
        #     self.timeToTurn -= 1
        # elif self.timeToTurn==0:
        #     self.timeSinceLastTurn = 0
        ...
        #simple speed limit - could be improved
        # if self.vl>10.0:
        #     self.vl = 10.0
        # if self.vl<-10.0:
        #     self.vl = -10.0
